...
weitergeben. Regeln stehen in [`AGENTS.md`](../AGENTS.md), Architektur in
[`docs/architecture.md`](architecture.md), Sicherheit in [`docs/security.md`](security.md).
...
Alles in §14 und [`docs/security.md`](security.md) ist die Ausformulierung dieses Satzes.
...
2. **Material ablegen.** Brief, Texte, PDFs, Bilder nach `input/` (Struktur:
   `input/brief.example.md`, `input/content/`, `input/documents/`, `input/images/`).
...
Vorlage: [`docs/seitenvorlage.md`](seitenvorlage.md). Beispiel „Methodik“:
...
Kurzfassung von [`docs/assets.md`](assets.md):

1. Material liegt in `input/images/`.
...
byte-identisch“ wäre ohne festes Container-Image eine Lüge. Dazu `docs/roadmap.md`
...
| `kompatibilitaet` | `22` (untere `engines`-Grenze) | `npm ci` → validate → build → test |
...
Ausführlich in [`docs/security.md`](security.md). Für die Arbeit vier Sätze:
...
5. **Dokumentation nachziehen:** `AGENTS.md` (Regel), `docs/architecture.md`, dieses
   Handbuch (§17 Diagnosen), `docs/roadmap.md`, falls etwas zurückgestellt wird.
...
`docs/seitenvorlage.md` → [`docs/assets.md`](assets.md) → dieses Handbuch.
...
Bewusst nicht enthalten, mit Begründung und Auslöser in [`docs/roadmap.md`](roadmap.md):
