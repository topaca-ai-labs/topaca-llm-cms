...
Zwei Quellen, ein Abgleich: [`docs/feedback.md`](feedback.md) (Kritik am Muster) und
[`docs/TOPACA LLM-CMS Minimalvariante – Architecture & Implementation Audit v0.1.md`](<TOPACA LLM-CMS Minimalvariante – Architecture & Implementation Audit v0.1.md>)
...
| „Drei-Schichten-Modell: `.gitignore` plus Konvention reicht" | Schichten in `AGENTS.md` §1, `.gitignore` für `dist/`, `.astro/`, `input/**`; nur die Grenze wird geprüft | `docs/architecture.md` §2 |
...
| „Machine-Web-Dual-Output später, `llms.txt` wird nicht unterstützt" | Kein zweites Ausgabeformat; Maschinenlesen durch semantisches HTML, `lang`, kanonischer Link, Sitemap, `robots.txt` | `docs/roadmap.md` |
| „Eigene Entwicklungsleistung bei etwa zehn Prozent" | 3 eigene Skripte, 1 Schema, 1 Routenmodul; Rest ist Astro und Text | `package.json`, `docs/architecture.md` §1 |
...
| P2-09 `public/assets/README.md` entfernen | gelöscht (es wäre als `/assets/README.md` ausgeliefert worden); Inhalt nach `docs/assets.md` verschoben; `FORBIDDEN_DIST_FILE` meldet jede `README.md` im Build | `docs/assets.md`, `tests/validator.test.mjs` → *interne Dokumentation im Auslieferungszustand* |
...
| Punkt | Grund |
| --- | --- |
| `llms.txt`, zweites Ausgabeformat | Standard ohne nennenswerte Nutzung; Maschine-Grundlagen sind vorhanden. `docs/roadmap.md` |
| Externer Link-Checker (`lychee`, `htmltest`) | Netzwerk und Zusatzinstallation in der Prüfung widersprechen R-12 |
| Bildpipeline (Komprimierung, `srcset`, AVIF) | echte Funktion, aber Umfang; Auslöser in `docs/roadmap.md` |
